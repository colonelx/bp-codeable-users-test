 ...
 * Users Datasource
 *
 * @package \BPCUT\Admin\UsersDatasource
 */
...
class UsersDatasource {
	...
	public function get_users() {
		if ( ! isset( $_GET['bpcut-nonce'] ) || ! wp_verify_nonce( $_GET['bpcut-nonce'], 'admin_url_users' ) ) {
			return;
		}
		$draw    = $_POST['draw'];
		...
		foreach ( $columns as $idx => $column ) {
			if ( self::ROLE_COLUMN_NAME === $column['name'] && ! empty( $column['search']['value'] ) ) {
